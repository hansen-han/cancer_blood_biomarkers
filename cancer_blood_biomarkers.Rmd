...
```{r}
attributes %>% filter(grepl("agilent", name))
```
...
#### Check if any samples are missing metadata/expr
```{r}
...
blood_biomarker_data_raw$originalData$GSE136651$expr <- log2(as.matrix(expr_data))
```
...
# log transform the data since it is in TPM format
blood_biomarker_data_raw$originalData$GSE120691$expr <- log2(as.matrix(expr_data))
```
...
# According to GEO, the data was processed with neqc which outputs data in log2 format, so it is good to go
# https://web.mit.edu/~r/current/arch/i386_linux26/lib/R/library/limma/html/nec.html
