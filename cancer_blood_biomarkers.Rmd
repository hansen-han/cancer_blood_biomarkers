...
blood_biomarker_data_raw$originalData$GSE136651$expr <- log2(as.matrix(expr_data) + 1)
```
...
train_ids <- shuffled_ids[1:(length(shuffled_ids) / 2)]
test_ids <- shuffled_ids[(length(shuffled_ids) / 2 + 1):length(shuffled_ids)]
...
Here, we look for a gene signature with an effect size > 1, significance of =< 0.05 and must be present in at least half the studies (3)
```{r}
meta_results <- filterGenes(meta_results, isLeaveOneOut = FALSE, effectSizeThresh = 1, FDRThresh = 0.05, numberStudiesThresh = 3)
```
...
Performance on discovery data is varied but overall 0.9 AUC
```{r}
