...
#### GSE136651 (Need to Download FASTQs)
...
```{r}
blood_biomarker_data_raw$originalData$GSE136651$exp_comment
```
...
```{r}
GSE136651_expr <- read_csv("GSE136651_panc_normal_normalized_counts_20230703.csv.gz")
GSE136651_expr
```
```{r}
GSE136651_meta <- read_csv("GSE136651_panc_normal_scaled_metaData_2023-05-18.csv.gz")
GSE136651_meta
```

```{r}
blood_biomarker_data_raw$originalData$GSE136651 <- NULL
```



#### GSE120691 (Need to Download FASTQs)
...
blood_biomarker_data_raw$originalData$GSE20189$pheno %<>% filter(description != "Case Excluded")
...
# save to the dataset
blood_biomarker_data_raw$originalData$GSE20189$expr <- expr_data
blood_biomarker_data_raw$originalData$GSE20189$keys <- gene_names
```

```{r}
blood_biomarker_data_raw$originalData$GSE20189$expr
```

There is a discrepancy between pheno and class samples 
```{r}
checkDataObject(blood_biomarker_data_raw$originalData$GSE20189, "Dataset")
```

```{r}
as.data.frame(blood_biomarker_data_raw$originalData$GSE20189$expr)
```
